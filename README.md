<img src="images/perfectpour_logo.png" width="350" alt="PerfectPour logo">
...
`Python` · `pandas` / `numpy` · `scikit-learn` (Linear Regression, Random Forest, Gradient Boosting, KNN) · `Plotly` / `Matplotlib` / `Seaborn` · `Gradio` (original prototype) · `Streamlit` (deployed live demo)
...
├── app/
│   ├── app.py                        # standalone PerfectPour Streamlit app
│   ├── requirements.txt
...
I used Claude (Anthropic) to restructure my original project report and analysis, already complete from coursework, into a more approachable, narrative-driven README case study. Since I knew the project inside and out from doing it with my partner, I reread the full README against my original report and outline to confirm it accurately reflected the analysis, results, and the points I wanted to highlight. I take full responsibility for the accuracy and content of this README and repository; all modeling results, findings, and conclusions are my own work from the original coursework. Turning this coursework into a public case study also gave me a low-risk way to intentionally build hands-on fluency with Claude, since I already had a clear end goal going in.
